...
namespace GTS.ToDoMgmt.Application.Todos.GetTodos
{
    public interface IGetTodosFactory
    {
        ...
        List<Expression<Func<Todo, bool>>> GetEntityFilters(GetTodosQuery getTodosQuery);
        GetTodosResponseDto GetTodosResponseDto(GetTodosQuery request, IEnumerable<Todo>? todos);
    }
    public class GetTodosFactory : IGetTodosFactory
    {
        public List<Expression<Func<Todo, bool>>> GetEntityFilters(GetTodosQuery getTodosQuery)
        {
            List<Expression<Func<Todo, bool>>> filters = [];
            if (getTodosQuery.TodoFilters?.StatusFilter != null)
                filters.Add(todo => todo.Status == Enum.Parse<TodoStatus>(getTodosQuery.TodoFilters.StatusFilter.ToString()));
            if (getTodosQuery.TodoFilters?.PriorityFilter != null)
                filters.Add(todo => todo.Priority == Enum.Parse<TodoPriority>(getTodosQuery.TodoFilters.PriorityFilter.ToString()));
